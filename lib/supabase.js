...
async function upsertInvoice(invoice) {
  const { data, error } = await supabase
    .from('invoices')
    .upsert(invoice, { onConflict: 'id', ignoreDuplicates: false })
    .select('id')
    .single();
...
// ─── Semantic search ────────────────────────────────────────────────────────
async function searchAll(embedding, options = {}) {
  const { threshold = 0.60, count = 15 } = typeof options === 'object' && !Array.isArray(options) ? options : { threshold: options, count: arguments[2] };
...
async function searchInvoices(embedding, options = {}) {
  const { threshold = 0.65, count = 10 } = typeof options === 'object' && !Array.isArray(options) ? options : { threshold: options, count: arguments[2] };
  const { data, error } = await supabase.rpc('match_invoices', {
    query_embedding: `[${embedding.join(',')}]`,
    match_threshold: threshold,
    match_count: count
  });
  if (error) throw new Error(`searchInvoices: ${error.message}`);
  return data || [];
}

async function searchDocuments(embedding, options = {}) {
  const { threshold = 0.60, count = 8 } = typeof options === 'object' && !Array.isArray(options) ? options : { threshold: options, count: arguments[2] };
